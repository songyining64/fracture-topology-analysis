...
地质断裂拓扑智能分析系统是一套面向地质工程与油气勘探的桌面端智能分析平台
<<<<<<< HEAD
本系统将断裂网络拓扑分析与机器学习相结合，实现从原始迹线数据到融合分析、预测与可解释性的完整流程，做到“看得清（可视化）—算得准（建模）—讲得明（解释）”。核心功能包括：
=======
>>>>>>> 7d154cba7b1ad7d91856256fca9c324afb5cd246

---

## 一、作品简介

本系统将**断裂网络拓扑分析**与**机器学习**相结合，实现从原始迹线数据到融合分析、预测与可解释性的完整流程。做到“看得清（可视化）—算得准（建模）—讲得明（解释）”。
---核心功能包括：

- **拓扑分析**：基于 fractopo 进行分支类型划分（CC / CI / II）与节点类型划分（X / Y / I），并在此基础上输出全局拓扑参数统计、方位集交叉与相邻关系分析及一系列成果；
